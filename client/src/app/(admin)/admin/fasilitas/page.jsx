...
export default function FasilitasPage() {
  ...
  const handleSubmit = async () => {
    if (!nama.trim()) {
      return showAlert("Nama fasilitas wajib diisi", "error");
    }
    ...
    try {
      if (isUpdate && editId) {
        await api.put(`/api/fasilitas/${editId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ fasilitas berhasil diperbarui", "success");
      } else {
        await api.post(`/api/fasilitas`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ Berhasil menambahkan fasilitas", "success");
      }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/api/fasilitas/${deleteId}`, {
        withCredentials: true,
      });
        ...
        <DialogContent className="max-w-md mx-auto rounded-xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-teal-700">
              {isUpdate ? "Update fasilitas" : "Tambah fasilitas"}
            </DialogTitle>
          </DialogHeader>
