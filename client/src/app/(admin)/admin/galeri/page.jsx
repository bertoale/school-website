...
export default function GaleriPage() {
  ...
  const handleSubmit = async () => {
    if (!judul.trim()) {
      return showAlert("Judul galeri wajib diisi", "error");
    }
    ...
    try {
      if (isUpdate && editId) {
        await api.put(`/api/galeri/${editId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ galeri berhasil diperbarui", "success");
      } else {
        await api.post(`/api/galeri`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ Berhasil menambahkan galeri", "success");
      }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/api/galeri/${deleteId}`, {
        withCredentials: true,
      });
        ...
        <DialogContent className="max-w-md mx-auto rounded-xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-teal-700">
              {isUpdate ? "Update galeri" : "Tambah galeri"}
            </DialogTitle>
          </DialogHeader>
