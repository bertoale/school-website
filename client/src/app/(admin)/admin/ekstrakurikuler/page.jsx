...
export default function EkstrakurikulerPage() {
  ...
  const getData = async () => {
    try {
      const res = await api.get(`/api/ekstrakurikuler`);
  ...
  const handleSubmit = async () => {
    if (!nama.trim()) {
      return showAlert("Nama ekstrakurikuler wajib diisi", "error");
    }
    ...
    try {
      if (isUpdate && editId) {
        await api.put(`/api/ekstrakurikuler/${editId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ Ekstrakurikuler berhasil diperbarui", "success");
      } else {
        await api.post(`/api/ekstrakurikuler`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
        showAlert("✅ Berhasil menambahkan ekstrakurikuler", "success");
      }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/api/ekstrakurikuler/${deleteId}`, {
        withCredentials: true,
      });
        ...
        <DialogContent className="max-w-md mx-auto rounded-xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-teal-700">
              {isUpdate ? "Update Ekstrakurikuler" : "Tambah Ekstrakurikuler"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <Input
              ...
              className="rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-200"
            />
